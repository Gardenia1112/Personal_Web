...
> 更新时间：2026-09-05 · 供 Cursor 接手者快速判断「现在到哪了、接下来干什么」。

---

## 1. 一句话状态

**Phase 1–6 骨架已完成、构建通过；手册 §6（横向视差画廊）、§3（Phase 1-2 精修）、§4（`/desktop` 改选择台）已落地，Phase 3（`/about`）重做为「碰旗弹卡」已提交，未做 Phase 7。**

- 已完成：Phase 1–6 骨架（工位、开场剧本、游戏化 About、桌面页、细节物件、彩蛋）。
- 已落地手册 §6（提交 `77f4e83`）：`/works` `/awards` 改横向视差画廊（D10）+ 全站渐黑转场（D5/D6）。
- 已落地手册 §3（Phase 1-2 精修，已提交）：首页接入 `room_full.glb` 实模、鼠标视角摇晃、昼夜双光照管线、hover 状态机改 GSAP 驱动；另加热点圆点当导航菜单（只给会跳路由的 4 件；台灯/咖啡不列，靠 hover 发光）、背景排版大字（画布透明、大字在房间之后做前后空间感）、提前落地手册 §7.1 的自定义光标，并删掉性能 HUD、跟随鼠标的重叠标签、投影在显示器上的「姓名 · 职位」。
...
- 🗑 **Resume 路由 D16 作废**：不建 `/resume` 页。简历入口先撤出 `/desktop`、挂到 `/about` 左上角；Phase 3 重做 `/about` 后**左上角下载链接也被撤掉**（`about.astro` 里 `profile` import 与 `.about-resume` 已删），现在**只有 `/contact`** 保留下载（`profile.resume` 字段仍在 `profile.ts`）。⚠️ 待确认是有意还是漏删——要恢复 `/about` 入口，在 `about.astro` 加回 `profile.resume` 链接即可。
...
- ⚠️ **`/about` 页图片偏大**：`public/assets/about/` 里 `chapter-*.png`、`now-desk-night.png` 等 30+ 张共约 15MB（多为 1-2MB 的大 PNG）。已入库、引用正常，但 Phase 7 上线前建议压成 WebP/JPEG 再回填，否则移动端首屏偏重。
