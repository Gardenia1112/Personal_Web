...
│   ├── 完整交接手册.md
│   ├── AGENTS.md        ← 旧版（以本文件为准）
│   └── PROJECT_STATE.md ← 旧版快照
├── references/          ← 参考素材（⚠️ 只能你手动放：pinterest/codrops/reactbits/recordings）
└── tasks/               ← 6 张任务卡（Phase 1-8 折叠成 6 张）
    ├── PHASE1-2-工位骨架与开场.md
    ├── PHASE3-游戏化介绍.md
    ├── PHASE4-桌面OS.md
    ├── PHASE5-细节物件.md
...
> 权威规范文档在仓库根 `docs/`；`handoff/docs/` 是其**只读副本**（内容一致）。要改设计，回根 `docs/` 改并同步。
> `完整交接手册.md` 是**给你的流程手册**（Claude→Cursor 交接步骤），Cursor 可不读；`references/` 里的参考图/录屏只能你手动放。
...
2. **英文命名**：代码文件/资源路径一律英文 slug，禁用中文名。
...
- `pinterest/` —— 视觉参考图（Pinterest 截图，喂给模型做风格对齐）。
- `codrops/` —— 交互/动效参考（Codrops demo 或代码片段）。
- `reactbits/` —— 窗口系统源码参考。
- `recordings/` —— 动效录屏（5-10s）。
- **每个参考配一个 `说明.md`**：链接 + 截图文件名 + 「我要复刻的具体部分」。这是给模型的关键上下文。
